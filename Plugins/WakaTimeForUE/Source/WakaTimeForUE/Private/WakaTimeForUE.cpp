...
void FWakaTimeForUEModule::SendHeartbeat(bool bFileSave, string Activity, string EntityType, FString Entity, string Language)
{
	...
	// Build a fixed command line matching the requested invocation.
	// Use the user's profile and project path variables where appropriate.
	string ExePath = string(GUserProfile) + "\\.wakatime\\wakatime-cli-windows-amd64.exe";

	string CmdLine = "\"" + ExePath + "\" ";
	CmdLine += "--config \"" + string(GUserProfile) + "\\.wakatime.cfg\" ";
	CmdLine += "--log-file \"" + string(GUserProfile) + "\\.wakatime\\wakatime.log\" ";
	// Use the fixed API URL requested
	CmdLine += "--api-url \"https://hackatime.hackclub.com/api/hackatime/v1\" ";
	CmdLine += "--project \"" + GetProjectName() + "\" ";
	CmdLine += "--project-folder \"" + GProjectPath + "\" ";
	CmdLine += "--entity \"Unreal Editor\" ";
	CmdLine += "--entity-type \"app\" ";
	CmdLine += "--plugin \"unreal-wakatime/" + GPluginVersion + "\" ";
	CmdLine += "--category \"" + Activity + "\" ";
	...
	try
	{
		// Execute the wakatime CLI directly (no Powershell wrapper). Pass the exe path as the application
		// and the full command line including the exe as the command to CreateProcess.
		bSuccess = FWakaTimeHelpers::RunCommand(CmdLine, false, ExePath, INFINITE, true);
	}
	catch (int Err)
	{
